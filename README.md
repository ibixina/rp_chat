...
The character tray above the composer lists every current member. Selecting a character prompts exactly that character to take the next turn without creating a fake user message; if a reply is selected, the chosen character receives that reply context.
