...
The character tray above the composer lists every current member and shows their individual presence indicator. Members stay green as available participants; the active character turns blue while generating. Selecting a character prompts exactly that character to take the next turn without creating a fake user message; the model receives only that character's identity, relationship context, group memory, and recent conversation—not tray or orchestration instructions. If a reply is selected, the chosen character receives that reply context. Group headers show member count rather than a misleading group-level online status.
